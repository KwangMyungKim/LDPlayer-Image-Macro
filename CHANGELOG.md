...
- **창 선택 기능**: 실행 중인 LDPlayer 창 목록을 검색하여 콤보박스(DropDownList)로 선택할 수 있습니다.
...
- **구조 개선**: 모든 텍스트를 `locales.ini` 파일로 분리하여 관리합니다. UTF-8 인코딩을 강제하여 문자 깨짐 현상을 해결했습니다.
...
### 기타 개선 사항
- **GUI 개선**: 편집 대화상자의 컨트롤 겹침 문제를 해결하고 높이를 최적화했습니다.
- **로그 개선**: 로그 줄바꿈 문자(`\r\n` -> `` `r`n ``)를 수정하고 `SendMessage` 방식을 사용하여 텍스트 굵어짐 현상을 해결했습니다.
